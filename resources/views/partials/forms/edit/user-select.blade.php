<div id="assigned_user" class="form-group{{ $errors->has($fieldname) ? ' has-error' : '' }}"{!!  (isset($style)) ? ' style="'.e($style).'"' : ''  !!}>
    ...
    <div class="col-md-7">
        <select class="js-data-ajax disabled" data-endpoint="users" data-placeholder="{{ trans('general.select_user') }}" {{ isset($disabled) && $disabled  ? 'disabled' : '' }} name="{{ $fieldname }}" style="width: 100%" id="assigned_user_select" aria-label="{{ $fieldname }}"{{  ((isset($required)) && ($required=='true')) ? ' required' : '' }}>
